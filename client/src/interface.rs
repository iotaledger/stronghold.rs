...
use crate::{
    actors::{
        ...
        GetAllClients, GetClient, GetSnapshot, GetTarget, RecordError, Registry, RemoveClient, SnapshotConfig,
        SpawnClient, SwitchTarget,
    },
...
impl Stronghold {
    ...
    pub async fn synchronize_local_partial(
        ...
        {
            Ok(result) => result,
            Err(error) => return Ok(Err(error)), // :thinking_face:
        };

        // load the current target
        if let Some(target) = self.registry.send(GetTarget {}).await? {
        ...
        Ok(Err(ReadError::CorruptedContent(format!(
            "No client present with id{:?}",
            id
        ))))
    }
