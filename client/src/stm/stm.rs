...
pub struct TxResult<T> {
    ...
    pub res: T,       // Return value of the transaction
}
...
impl Stm {
    ...
    pub fn read_write<F, U>(&self, transaction: F) -> Result<TxResult<U>, TxError>
    where
        F: Fn(&mut Transaction) -> Result<U, TxError>,
    {
        ...
        loop {
            let mut tx = Transaction::new(self.get_clock(), tx_id);
    ...
    pub fn read_only<F, U>(&self, transaction: F) -> Result<TxResult<U>, TxError>
    where
        F: Fn(&mut Transaction) -> Result<U, TxError>,
    {
        let tx_id = self.increment_tx_ids();
