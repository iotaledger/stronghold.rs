...
use crate::{
    ...
    locked_memory::{MemoryError::*, MemoryType::Ram, *},
...
// We currently only implement for u8 because our encryption functions only returns Vec[u8], therefore our cipher is
// Buffer<u8>
...
impl<P: BoxProvider> ZeroizeOnDrop for EncryptedRam<P> {}
...
mod tests {
    ...
    fn test_lock_unlock() {
        let key = Key::random();
        let ram = EncryptedRam::<Provider>::alloc(
            &[1, 2, 3, 4, 5, 6][..],
            6,
            LockedConfiguration {
                mem_type: Ram,
        ...
        assert!(ram.is_ok());
        let ram = ram.unwrap();
        let buf = ram.unlock(LockedConfiguration {
            mem_type: Ram,
            encrypted: (Some(key.clone())),
        });
        assert!(buf.is_ok());
