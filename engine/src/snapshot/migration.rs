...
pub fn read_wallet_snapshot(path: &Path, password: &[u8], aad: &[u8],) -> Result<Vec<u8>, Error> {
    ...
    const MIN_LEN: u64 = (MAGIC.len() + VERSION_V2.len() + x25519::PUBLIC_KEY_LENGTH + XChaCha20Poly1305::TAG_LENGTH) as u64;
...
pub fn read<I: Read>(input: &mut I, key: &Key, _associated_data: &[u8]) -> Result<Vec<u8>, Error> {
    ...
    age::decrypt_vec(key, &age[..])
        .map_err(|e| Error::AgeError(e))
}
...
pub fn migrate(prev: Version, next: Version) -> Result<(), Error> {
    match (prev, next) {
        ( Version::V2wallet { path: prev_path, password: prev_pwd, aad: prev_aad, }
        , Version::V3 { path: next_path, password: next_pwd, aad: next_aad, }
