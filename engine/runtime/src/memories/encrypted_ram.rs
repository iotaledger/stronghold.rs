...
use crate::{
    ...
    locked_memory::{LockedConfiguration::*, MemoryError::*, ProtectedConfiguration::*, *},
...
impl<P: BoxProvider> LockedMemory<u8, P> for EncryptedRam<P> {
        ...
        match config {
            ...
            EncryptedRam(key, size) => {
                ...
                Ok(EncryptedRam {
                    ...
                    config: EncryptedRam(Key::random(), size),
                    ad,
                })
            }
    ...
    fn lock(mut self, payload: Buffer<u8>, config: LockedConfiguration<P>) -> Result<Self, MemoryError> {
        match config {
            EncryptedRam(_, _) => {
                self.dealloc().expect("Deallocate locked memory failed");
    ...
    fn unlock(&self, config: LockedConfiguration<P>) -> Result<Buffer<u8>, MemoryError> {
        ...
        if let EncryptedRam(key, None) = config {
            ...
            if let EncryptedRam(_, Some(size)) = self.config {
...
mod tests {
    ...
    fn test_lock_unlock() {
        let key = Key::random();
        let ram = EncryptedRam::<Provider>::alloc(&[1, 2, 3, 4, 5, 6][..], EncryptedRam(key.clone(), Some(6)));
        assert!(ram.is_ok());
        let ram = ram.unwrap();
        let buf = ram.unlock(EncryptedRam(key.clone(), None));
        assert!(buf.is_ok());
        let buf = buf.unwrap();
        assert_eq!((*buf.borrow()), [1, 2, 3, 4, 5, 6]);
        let ram = ram.lock(buf, EncryptedRam(key, Some(6)));
        assert!(ram.is_ok());
    }
    ...
    fn test_crypto() {
        let key = Key::random();
        let ram = EncryptedRam::<Provider>::alloc(&[1, 2, 3, 4, 5, 6][..], EncryptedRam(key, Some(6)));
        assert!(ram.is_ok());
        let ram = ram.unwrap();
