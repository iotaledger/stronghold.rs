...
//! solved for stronghold by using a mailbox concept that is described later.
//!
...
//! what this library is doing. Stronghold-communication uses multicast DNS (mDNS) for peer discovery in a local network
//! and the RequestResponse protocol in order to send / receive custom messages and parse them.
//!
//! ## Multiplexing and Noise-encryption
//!
//! The transport of stronghold-communication is upgraded with yamux for multiplexing and the noise protocol, this noise
//! protocol uses the XX-Handshake and ensures authentification and encryption.
//!
...
//! sending messages and reacting upon the outcome of the operation. Upon creating a new instance, a transport is
//! created and upgraded, and combined with a the P2PNetworkBehaviour into a ExpandedSwarm. This Swarm is returned to
//! the caller and serves as entrypoint for all communication to other peers. It implements methods for listening to the
//! swarm, sending outbound messages, and manually adding and dialing peers. Incoming `P2PEvent` can be handled by
//! polling from the swarm, e.g. via the `poll_next_unpin` method. Due to libp2ps concept of `multiaddresses`, the swarm
//! has multiple listening addresses that encode different addressing schemes for different protocols. Apart from IPv4
//! and IPv6 Addresses, these multiaddresses can also be dns addresses, which is relevant if a peer is listening to such
//! an address on a server. The listed multiaddresses are only the ones within the same local network, but if port
//! forwarding was configured, the local /ip4/my-local-address/tcp/12345 Address can be replaced by the public one or by
...
//! `Request::PutRecord` message with the record to the mailbox, and that can then return the Records to remote peers
//! upon receiving a `Request::GetRecord` request.
