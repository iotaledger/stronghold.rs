...
mod test {
    ...
    fn start_listening(swarm: &mut Swarm<P2PNetworkBehaviour<String, String>>) -> Multiaddr {
        task::block_on(async {
            loop {
                match swarm.next_event().await {
                    ...
                    | SwarmEvent::ListenerError { error: _ } => panic!("Could not start listening."),
                    _ => {}
                }
            }
        })
    ...
    fn listen_addr() {
        ...
        let actual_addr = start_listening(&mut swarm);
    ...
    fn zeroed_addr() {
        ...
        let mut actual_addr = start_listening(&mut swarm);
