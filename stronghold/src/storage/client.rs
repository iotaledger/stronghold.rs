...
use crate::{
    line_error,
    storage::connection::{send_until_success, CRequest, CResult},
    storage::state::State,
};
...
impl<P: BoxProvider + Send + Sync + 'static> Client<P> {
    ...
    pub fn write(&self, id: RecordId, hint: Option<RecordHint>, payload: Vec<u8>) {
        self.db.take(|db| {
            let mut reqs = vec![];
            ...
            reqs.append(
                &mut w
                    .write(
                        &payload,
                        hint.unwrap_or_else(|| RecordHint::new(b"").expect(line_error!())),
                    )
                    .expect(line_error!()),
            );
    ...
    pub fn get_index(&self) -> Vec<(RecordId, RecordHint)> {
    ...
    pub fn read_record_by_id(&self, id: RecordId) -> String {
        self.db.take(|db| {
