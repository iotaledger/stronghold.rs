...
use parti::vault::{BoxProvider, Key};
...
impl BoxProvider for Provider {
    ...
    fn box_seal(key: &Key<Self>, ad: &[u8], data: &[u8]) -> parti::vault::Result<Vec<u8>> {
        ...
        XChaChaPoly
            ...
            .map_err(|_| parti::vault::Error::CryptoError(String::from("Unable to seal data")))?;
        Ok(boxx)
    }

    // open the box with the key.
    fn box_open(key: &Key<Self>, ad: &[u8], data: &[u8]) -> parti::vault::Result<Vec<u8>> {
        let mut plain = match data.len() {
            len if len >= Self::box_overhead() => vec![0; len - Self::box_overhead()],
            _ => return Err(parti::vault::Error::CryptoError(String::from("Truncated cipher"))),
        };
        ...
        XChaChaPoly
            ...
            .map_err(|_| parti::vault::Error::CryptoError(String::from("Invalid Cipher")))?;

        Ok(plain)
    }

    // create a random buffer
    fn random_buf(buf: &mut [u8]) -> parti::vault::Result<()> {
        OsRng
            .random(buf)
            .map_err(|_| parti::vault::Error::CryptoError(String::from("Can't generated random Bytes")))
    }
}
